'use client'

import { useEffect, useState } from 'react'
import { useRouter } from 'next/navigation'
import { supabase } from '@/lib/supabase'
import { getActorCount } from '@/lib/queries'

export default function LoginPage() {
  const router = useRouter()
  const [email, setEmail] = useState('')
  const [password, setPassword] = useState('')
  const [remember, setRemember] = useState(false)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState('')
  const [actorCount, setActorCount] = useState<number | null>(null)

  useEffect(() => {
    getActorCount().then(setActorCount).catch(() => {})
  }, [])

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    setLoading(true)
    setError('')
    const { error: err } = await supabase.auth.signInWithPassword({ email, password })
    if (err) {
      setError(err.message)
      setLoading(false)
    } else {
      router.push('/dashboard')
    }
  }

  async function handleOAuth(provider: 'google' | 'github') {
    await supabase.auth.signInWithOAuth({
      provider,
      options: { redirectTo: `${window.location.origin}/dashboard` },
    })
  }

  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', height: '100vh' }}>
      {/* ── Left: brand panel ── */}
      <div className="login-brand">
        <div className="orb orb-1"></div>
        <div className="orb orb-2"></div>
        <div className="orb orb-3"></div>
        <div className="mesh"></div>

        <div className="brand-top">
          <div className="brand-logo-icon">
            <svg viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="1.8">
              <path d="M12 3c-1.2 3.6-4.4 6-8 7 1.6 2 2.8 4.6 3 7.5C8.6 15.2 10.2 13 12 12c1.8 1 3.4 3.2 5 5.5.2-2.9 1.4-5.5 3-7.5-3.6-1-6.8-3.4-8-7z"/>
            </svg>
          </div>
          <span className="brand-logo-name">VerdeXcelerate</span>
        </div>

        <div className="brand-middle">
          <h1>Mapeo Vivo del Ecosistema AgrifoodTech</h1>
          <p>6 paises &middot; 9 cadenas de valor &middot; datos en tiempo real</p>
        </div>

        <div className="float-badge float-badge-1">
          <span className="float-badge-num">{actorCount ?? '—'}</span>
          <span className="float-badge-label">actores mapeados</span>
        </div>
        <div className="float-badge float-badge-2">
          <span className="float-badge-num">6</span>
          <span className="float-badge-label">paises</span>
        </div>

        <div className="brand-stats">
          <div className="brand-stat">
            <div className="brand-stat-num">{actorCount ?? '—'}</div>
            <div className="brand-stat-label">Actores</div>
          </div>
          <div className="brand-stat">
            <div className="brand-stat-num">6</div>
            <div className="brand-stat-label">Paises</div>
          </div>
          <div className="brand-stat">
            <div className="brand-stat-num">9</div>
            <div className="brand-stat-label">Cadenas</div>
          </div>
        </div>
      </div>

      {/* ── Right: form panel ── */}
      <div className="login-form-panel">
        <div className="login-form-inner">
          <div className="pill-badge">Beta &middot; Acceso anticipado</div>
          <h2>Iniciar sesion</h2>
          <p className="subtitle">Accede al Mapeo Vivo AgrifoodTech</p>

          <button className="btn-oauth" type="button" onClick={() => handleOAuth('google')}>
            <svg viewBox="0 0 24 24" width="18" height="18">
              <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
              <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
              <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
              <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
            </svg>
            Continuar con Google
          </button>

          <button className="btn-oauth" type="button" onClick={() => handleOAuth('github')}>
            <svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor">
              <path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z"/>
            </svg>
            Continuar con GitHub
          </button>

          <div className="divider"><span>o con correo</span></div>

          <form onSubmit={handleSubmit}>
            {error && (
              <div className="login-error">{error}</div>
            )}
            <div className="field-group">
              <label htmlFor="email">Correo electronico</label>
              <input
                id="email"
                type="email"
                placeholder="tu@organizacion.org"
                value={email}
                onChange={e => setEmail(e.target.value)}
                required
                autoComplete="email"
              />
            </div>
            <div className="field-group">
              <label htmlFor="password">Contrasena</label>
              <input
                id="password"
                type="password"
                placeholder="••••••••"
                value={password}
                onChange={e => setPassword(e.target.value)}
                required
                autoComplete="current-password"
              />
            </div>
            <div className="login-row">
              <label className="checkbox-label">
                <input
                  type="checkbox"
                  checked={remember}
                  onChange={e => setRemember(e.target.checked)}
                />
                Recordarme
              </label>
              <a href="#" className="forgot-link">Recuperar contrasena</a>
            </div>
            <button type="submit" className="btn-primary" disabled={loading}>
              {loading ? 'Iniciando...' : 'Iniciar sesion →'}
            </button>
          </form>

          <div className="login-footer">
            <p>Al acceder aceptas los <a href="#">Terminos de uso</a> y la <a href="#">Politica de privacidad</a>.</p>
          </div>
          <div className="trusted-strip">
            <span className="trusted-label">Usado por</span>
            <span className="trusted-org">TechnoServe</span>
            <span className="trusted-sep">&middot;</span>
            <span className="trusted-org">BID Lab</span>
          </div>
        </div>
      </div>
    </div>
  )
}
